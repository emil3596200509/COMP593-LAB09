""" 
Name = Emil Mathew George
Partners = Krishna Prakash, Shanad,Alen Mulangan Davi
Description: 
...
def handle_btn_get_info():
    ...
    if poke_info:
        ...
        bar_splattack['value'] = poke_info['stats'][3]['base_stat']
        bar_spldefense['value'] = poke_info['stats'][4]['base_stat']
        bar_speed['value'] = poke_info['stats'][5]['base_stat']
    else:
        error_message ="Unable to fetch information for {} from the PokeAPI.".format(poke_name)
...
btn_get_info.grid(row=0, column=2, padx=(10,10), pady=10)
...
lbl_height_val.grid(row=0, column=1, padx=(0,10), pady=10)
...
lbl_weight_val.grid(row=1, column=1, padx=(0,10), pady=10)
...
lbl_type_val.grid(row=2, column=1, padx=(0,10), pady=10)
...
lbl_defence = ttk.Label(frm_stats, text="Defense: ")
...
lbl_splattack.grid(row=3, column=0, padx=(10,5), pady=5, sticky=E)
bar_splattack = ttk.Progressbar(frm_stats, length=PRG_BAR_LENGTH, maximum=STAT_MAX_VALUE)
bar_splattack.grid(row=3, column=1, padx=(0,10), pady=5)

lbl_spldefence = ttk.Label(frm_stats, text="Special Defense: ")
lbl_spldefence.grid(row=4, column=0, padx=(10,5), pady=5, sticky=E)
bar_spldefense = ttk.Progressbar(frm_stats, length=PRG_BAR_LENGTH, maximum=STAT_MAX_VALUE)
bar_spldefense.grid(row=4, column=1, padx=(0,10), pady=5)
